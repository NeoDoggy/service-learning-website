import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';
import 'package:provider/provider.dart';
import 'package:service_learning_website/modules/my_router.dart';
import 'package:service_learning_website/pages/page_skeleton.dart';
import 'package:service_learning_website/providers/activities_provider.dart';
import 'package:service_learning_website/providers/auth_provider.dart';
import 'package:service_learning_website/providers/courses_provider.dart';
import 'package:service_learning_website/widgets/online_course_card.dart';
import 'package:service_learning_website/widgets/title_text_box.dart';

class BackstagePage extends StatelessWidget {
  const BackstagePage({super.key});

  @override
  Widget build(BuildContext context) {
    final authProvider = Provider.of<AuthProvider>(context);
    if (!authProvider.isAuthed) {
      return const Scaffold(body: Center(child: Text("Loading")));
    }
    final userData = authProvider.userData!;

    return PageSkeleton(
      body: Consumer2<CoursesProvider, ActivitiesProvider>(
        builder: (context, coursesProvider, activitiesProvider, child) {
          final coursesData = coursesProvider.coursesData;
          final activitiesData = activitiesProvider.activitiesData;

          return Column(
              mainAxisAlignment: MainAxisAlignment.start,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const TitleTextBox("我的營隊"),
                const SizedBox(height: 60),
                LayoutBuilder(
                  builder: (context, constraint) {
                    const double cardWidth = 350;
                    final colWidth = constraint.maxWidth;
                    int cardCount;
                    for (cardCount = 0;
                        colWidth >=
                            cardWidth * (cardCount + 1) + 40 * cardCount;
                        cardCount++) {}
                    return Wrap(
                      spacing:
                          (colWidth - cardWidth * cardCount) / (cardCount - 1),
                      runSpacing: 60,
                      children: [
                        // for (var activityData in activitiesData.values.where(
                        //     (e) => userData.joinedActivities.contains(e.id)))
                        for (var activityData in activitiesData.values)
                          OnlineCourseCard(
                            onTap: () => context.go(
                                "/${MyRouter.backstage}/${MyRouter.activities}/${activityData.id}"),
                            width: cardWidth,
                            imageUrl: activityData.imageUrl,
                            courseName: activityData.title,
                          )
                      ],
                    );
                  },
                ),
                const SizedBox(height: 60),
                const TitleTextBox("我的課程"),
                const SizedBox(height: 60),
                LayoutBuilder(
                  builder: (context, constraint) {
                    const double cardWidth = 350;
                    final colWidth = constraint.maxWidth;
                    int cardCount;
                    for (cardCount = 0;
                        colWidth >=
                            cardWidth * (cardCount + 1) + 40 * cardCount;
                        cardCount++) {}
                    return Wrap(
                      spacing:
                          (colWidth - cardWidth * cardCount) / (cardCount - 1),
                      runSpacing: 60,
                      children: [
                        for (var courseData in coursesData.values.where(
                            (e) => userData.joinedCourses.contains(e.id)))
                          OnlineCourseCard(
                            onTap: () => context
                                .go("/${MyRouter.courses}/${courseData.id}"),
                            width: cardWidth,
                            imageUrl: courseData.imageUrl,
                            courseName: courseData.title,
                          )
                      ],
                    );
                  },
                ),
                const SizedBox(height: 60),
              ]);
        },
      ),
    );
  }
}
